...
{
    std::vector<float> vec( _eigmat.data(), _eigmat.data() + _eigmat.size() );//由data个元素，每个大小是data+size，就是将二维矩阵变成一维向量
    return vec;
} // eig2stdvec
...
double SCManager::distDirectSC ( MatrixXd &_sc1, MatrixXd &_sc2 )
{
    ...
    for ( int col_idx = 0; col_idx < _sc1.cols(); col_idx++ )
    {
        ...
        double sector_similarity = col_sc1.dot(col_sc2) / (col_sc1.norm() * col_sc2.norm());//计算相似度

        sum_sector_similarity = sum_sector_similarity + sector_similarity;
        num_eff_cols = num_eff_cols + 1;
    }
    
    double sc_sim = sum_sector_similarity / num_eff_cols;//求所有列的相似的的平均值
    return 1.0 - sc_sim;//返回偏差
...
int SCManager::fastAlignUsingVkey( MatrixXd & _vkey1, MatrixXd & _vkey2)
{
    ...
    for ( int shift_idx = 0; shift_idx < _vkey1.cols(); shift_idx++ )//一个一个移位，找最匹配的
    {
...
std::pair<double, int> SCManager::distanceBtnScanContext( MatrixXd &_sc1, MatrixXd &_sc2 )
{
    ...
    for ( int num_shift: shift_idx_search_space )
    {
        ...
        if( cur_sc_dist < min_sc_dist )//找偏差最小的
        {
...
MatrixXd SCManager::makeScancontext( pcl::PointCloud<SCPointType> & _scan_down )
{
    ...
    for (int pt_idx = 0; pt_idx < num_pts_scan_down; pt_idx++)
    {
        ...
        pt.z = _scan_down.points[pt_idx].z + LIDAR_HEIGHT; // naive adding is ok (all points should be > 0).将点云存储
        ...
        ring_idx = std::max( std::min( PC_NUM_RING, int(ceil( (azim_range / PC_MAX_RADIUS) * PC_NUM_RING )) ), 1 );//ceil：取整，求最大的范围
        sctor_idx = std::max( std::min( PC_NUM_SECTOR, int(ceil( (azim_angle / 360.0) * PC_NUM_SECTOR )) ), 1 );

        // taking maximum z 
        if ( desc(ring_idx-1, sctor_idx-1) < pt.z ) // -1 means cpp starts from 0 desc:存储最大高度
...
std::pair<int, float> SCManager::detectLoopClosureIDBetweenSession (std::vector<float>& _curr_key, Eigen::MatrixXd& _curr_desc)
{
    ...
    for ( int candidate_iter_idx = 0; candidate_iter_idx < NUM_CANDIDATES_FROM_TREE; candidate_iter_idx++ )
    {
        ...
        double candidate_dist = sc_dist_result.first;//最小匹配距离
        int candidate_align = sc_dist_result.second;//偏移量

        if( candidate_dist < min_dist )//找所有candidate里距离最小的
        {
...
std::pair<int, float> SCManager::detectLoopClosureID ( void )
{
     ...
     */
    if( (int)polarcontext_invkeys_mat_.size() < NUM_EXCLUDE_RECENT + 1)//太早了，先别做回环检测了？
    {
    ...
    {
        TicTocV2 t_tree_construction;//计时

        polarcontext_invkeys_to_search_.clear();
        polarcontext_invkeys_to_search_.assign( polarcontext_invkeys_mat_.begin(), polarcontext_invkeys_mat_.end() - NUM_EXCLUDE_RECENT ) ;//将polarcontext_invkeys_to_search_中的内容替代为polarcontext_invkeys_mat_中的内容
